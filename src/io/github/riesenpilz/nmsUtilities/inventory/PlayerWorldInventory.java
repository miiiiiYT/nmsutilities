...
public class PlayerWorldInventory {
    ...
    public JsonObject toJson() {
        ...
        for (ItemStack itemStack : getContents())
            jsonArray.add(itemStack == null ? "" : new io.github.riesenpilz.nmsUtilities.inventory.ItemStack(itemStack).toNBTTag().toString());
            ...
            for (int i = 0; i < jsonArray.size(); i++)
				try {
					contents[i] = jsonArray.get(i).getAsString().isEmpty() ? null
					        : io.github.riesenpilz.nmsUtilities.inventory.ItemStack.getItemStack(new NBTTag(jsonArray.get(i).getAsString())).getBukkit();
